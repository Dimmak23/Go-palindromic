...
int get_reverse_bin(int k[], int rev[], int digits) {
    for (int i=0;i<digits;i++) rev[i]=k[digits-i-1];
}
...
// Declaring and set higher number for research
    ...
    std::cout<<"Double-base palindromic numbers:\n\n";
...
// Part of programm with searching for double-base palindromic numbers.
    for (i=1;i<N;i++) {
        ...
        for (j=0;j<digits;j++)
            if (br[j]!=b[j]) goto END;
        
        // Print result.
        std::cout << "#" << ++z << ". Decimal: "<< i <<"; Binary: ";
        for (j=0;j<digits;j++) std::cout<<b[j];
        std::cout<<";\n";
        sum+=i;
...
// Print sum of all double-base palindromic numbers bellow N.
    std::cout << "\nSum of double-base palindromic numbers bellow " << N
              << " is = " << sum << ".\n";
